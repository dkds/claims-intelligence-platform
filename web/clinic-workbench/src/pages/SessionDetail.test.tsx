...
describe('SessionDetail', () => {
  ...
  it('verifies a logged session and navigates back to the sessions list', async () => {
    vi.mocked(sessionsApi.verifySession).mockResolvedValue({ ...baseSession, status: 'VERIFIED' })
    renderWithQueryClient(<SessionDetail />)

    const verifyButton = await screen.findByRole('button', { name: /verify session/i })
    await userEvent.click(verifyButton)

    await waitFor(() => {
      expect(sessionsApi.verifySession).toHaveBeenCalledWith('session-1', 'mgr-1')
      expect(mockNavigate).toHaveBeenCalledWith('/sessions')
    })
  })

  it('shows an error message when verification fails', async () => {
    vi.mocked(sessionsApi.verifySession).mockRejectedValue(new Error('downstream failure'))
    renderWithQueryClient(<SessionDetail />)
