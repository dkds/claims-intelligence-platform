...
export function Dashboard() {
  ...
  const adjudicated = claims?.filter(c => c.status === 'ADJUDICATED').length ?? 0
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {user?.role === 'clinic_manager' && (
          <>
            <StatCard label="Total sessions" value={sessions?.length ?? 0} to="/sessions" />
            <StatCard
        ...
        <StatCard label="Total claims" value={claims?.length ?? 0} to="/claims" />
        <StatCard
