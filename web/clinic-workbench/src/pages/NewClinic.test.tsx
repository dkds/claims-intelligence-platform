...
describe('NewClinic', () => {
  ...
  it('registers a clinic with trimmed fields and navigates to the clinics list', async () => {
    vi.mocked(enrollmentApi.createClinic).mockResolvedValue({ _id: 'clinic-1' })
    renderWithQueryClient(<NewClinic />)

    const [nameInput] = screen.getAllByRole('textbox')
    await userEvent.type(nameInput, ' Riverside Vets ')
    await userEvent.click(screen.getByRole('button', { name: /register clinic/i }))
  ...
  it('shows an error message when registration fails', async () => {
    vi.mocked(enrollmentApi.createClinic).mockRejectedValue(new Error('downstream failure'))
    renderWithQueryClient(<NewClinic />)
