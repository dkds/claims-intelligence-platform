...
describe('ClaimDetail', () => {
  ...
  it('approves a pending claim', async () => {
    vi.mocked(claimsApi.approveClaim).mockResolvedValue({ ...baseClaim, status: 'APPROVED' })
  ...
  it('rejects a pending claim with the entered reason', async () => {
    vi.mocked(claimsApi.rejectClaim).mockResolvedValue({ ...baseClaim, status: 'REJECTED' })
    renderWithQueryClient(<ClaimDetail />)

    await userEvent.click(await screen.findByRole('button', { name: /^reject$/i }))
    await userEvent.type(screen.getByPlaceholderText(/reason for rejection/i), 'Duplicate submission')
    await userEvent.click(screen.getByRole('button', { name: /confirm reject/i }))

    await waitFor(() => {
      expect(claimsApi.rejectClaim).toHaveBeenCalledWith('claim-1', 'adj-1', 'Duplicate submission')
    })
  })

  it('shows an error message when the approve action fails', async () => {
    vi.mocked(claimsApi.approveClaim).mockRejectedValue(new Error('downstream failure'))
    renderWithQueryClient(<ClaimDetail />)
