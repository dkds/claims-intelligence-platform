...
async function fillRequiredFields() {
  ...
  await userEvent.type(
    screen.getByPlaceholderText('e.g. DENT-CLEAN'),
    'DENT-CLEAN',
  );
...
describe('NewClaim', () => {
  ...
  it('submits a manual claim with trimmed and parsed line values, then navigates to the claims list', async () => {
    ...
    await waitFor(() => {
      expect(claimsApi.submitClaim).toHaveBeenCalledWith('clinic-1', {
        ...
        submittedBy: 'mgr-1',
        lines: [
          { procedureCode: 'DENT-CLEAN', quantity: 1, requestedAmount: 49.99 },
        ],
      });
  ...
  it('shows an error message when submission fails', async () => {
    vi.mocked(claimsApi.submitClaim).mockRejectedValue(
    ...
    expect(
      await screen.findByText('Submission failed. Check IDs and try again.'),
    ).toBeInTheDocument();
